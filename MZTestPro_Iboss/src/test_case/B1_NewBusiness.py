...
class NewBusinessTest(myunit.MyTest):
    '''商务新增商机测试'''
    
    def tearDown(self):
        #用例执行完成后暂时不用退出，下一个用例需要用本次用例的结果
        pass
    
        
    #测试商务进入谈单下单页面
    def Business_NewBusiness(self):
        self.page_NewBusiness = NewBusiness(self.driver)
        self.page_NewBusiness.NewBusiness(1,"4222787","666666")

    
    
    def test_BusinessCheck(self):
        '''新增订单'''
        '''(1)点击"新增商机"按钮，进入商机验证页面
           (2)填写用户信息，进入正式下单页面
           (3)填写订单信息，确认下单 '''
